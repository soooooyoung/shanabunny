...
const Header = () => {
  ...
  return (
    <header
      className={`top-0 z-40 mx-auto py-10 px-3 w-full flex-none bg-transparent transition-all duration-100 ease-in dark:bg-slate-900 md:bg-white/90 md:backdrop-blur-sm dark:md:bg-slate-900/90 ${
            ...
            {links &&
              links.map(({ label, href, icon: Icon, links }, index) => (
                ...
                >
                  {
                    <Link
                      className="flex items-center px-4 py-3 font-medium transition duration-150 ease-in-out hover:text-purple-300 dark:hover:text-white"
                      href={href as string}
                      onClick={() =>
                        isToggleMenuOpen
                          ? handleToggleMenuOnClick()
                  ...
                  {/* {links && links.length ? (
                    <>
                      <button
                        className="flex items-center px-16 py-3 font-medium transition duration-150 ease-in-out hover:text-purple-300 dark:hover:text-white"
                        onClick={() => handleDropdownOnClick(index)}
                      >
                        {label}{" "}
                        {Icon && (
                          <Icon className="ml-0.5 hidden md:inline size-8 center" />
                        )}
                      </button>
                      <ul
                        className={`${
                          isDropdownOpen[index] ? "block" : "md:hidden"
                        } rounded pl-4 font-medium  md:absolute md:min-w-[150px] md:bg-white/90 md:pl-0 md:backdrop-blur-md dark:md:bg-slate-900/90`}
                      >
                        {links.map(({ label: label2, href: href2 }, index2) => (
                          <li key={`item-link-${index2}`}>
                            <Link
                              className="whitespace-no-wrap block py-2 px-12 first:rounded-t last:rounded-b dark:hover:bg-gray-700 md:hover:bg-rose-200
                              md:hover:text-white"
        ...
        >
          <div className="flex w-full items-center justify-between md:w-auto">
            {actions && actions.length > 0 && (
              <div className="ml-4 flex w-max flex-wrap justify-end text-sky-200">
